...
- Интерфейс iOS-клиента сближен с Android: применена единая светлая/тёмная палитра, размеры
  шапки, собственный pill-переключатель вкладок, карточки профилей и меню действий. Кнопки
  «Импорт», «Новый» и «Проверить все» используют адаптивную одно-/двухрядную компоновку и не
  обрезают русские подписи на узких экранах и при Dynamic Type; перемещение профилей доступно
  через понятные пункты меню «вверх/вниз».
- iOS-клиент до чтения профилей проверяет фактические entitlements своей установленной подписи.
  Некорректно переподписанный unsigned IPA теперь показывает одно понятное сообщение о
  недостающих Network Extension, App Group или Keychain Sharing и не выводит подряд ошибки
  `Missing required entitlement` и `VPN configuration: permission denied`. Добавлен
  `scripts/verify_ios_ipa.py`: на macOS он fail-closed проверяет подписи приложения, Packet Tunnel
  и Widget extensions, provisioning profiles, согласованность идентификаторов и итоговые
  entitlements; приложенный к issue #160 неподписанный IPA ожидаемо отклоняется.

- Исправлен запуск Windows per-app split tunnel (`only selected applications` и
  `except applications`) на системах с виртуальными, WFP/QoS или IPv6-only интерфейсами.
  Определение физически подключённых IPv4-подсетей теперь заранее проверяет поддержку IPv4,
  изолирует ошибку каждого адаптера и продолжает запуск с исправными интерфейсами. Ошибка
  `NetworkInformationException (10043)` одного служебного адаптера больше не отклоняет весь
  `NetworkPlan`; неизвестная локальная сеть остаётся в fail-closed туннельном пути. Та же
  защита применяется к общему desktop-коду Windows и macOS.

- macOS-клиент теперь восстанавливает системные DNS, маршруты и исходное состояние
  `ip.forwarding` до закрытия дескриптора `utun`. Это устраняет задержку отключения примерно
  на 20 секунд, когда уничтожение интерфейса запускало DHCP-проверки `configd` и блокировало
  `networksetup`. Неудачное восстановление остаётся доступным для повторной попытки в
  `CleanupPlatform`, а длительность успешного возврата DNS записывается в журнал.

- Исправлены пограничные сценарии roaming/multipath и генерации профилей: TCP-клиент теперь
  закрепляет потоки за стабильными logical slot ID, поэтому потеря или восстановление одного
  bonded carrier не переназначает здоровые потоки; iOS сериализует неотменяемые
  `NEPacketTunnelFlow.readPackets` между поколениями и передаёт поздний первый пакет новому
  соединению; Android считает порядок DNS-серверов частью NetworkPlan и пересоздаёт TUN при
  смене primary/secondary resolver. Clone Profile в панели очищает дополнительные listeners,
  IPv4/IPv6 reservations и DHCP-диапазон, учитывает порты `bind.listen` и пересечение IPv6-
  префиксов. Quick Start показывает фактически сохранённые transport/port. iOS также распознаёт
  TCP/UDP за IPv6 extension headers при выборе срока хранения handoff-пакетов.

- Android-клиент сохраняет диагностический журнал между отключениями, пересозданием Activity и
  выгрузкой интерфейса: VPN-сервис записывает до 1000 последних событий (не более 512 КиБ) в
  приватный no-backup файл, а вкладка «Журнал» восстанавливает его при открытии. Только явная
  кнопка «Очистить» удаляет историю. Каждая пользовательская VPN-сессия получает идентификатор,
  сохраняются причины остановки, redelivery, отзыв VPN Android и предупреждение об оптимизации
  батареи; уровни Debug/Trace дополнительно фиксируют выключение и включение экрана. Активное
  ручное подключение теперь, как Always-on и Trusted Wi-Fi, возвращает `START_REDELIVER_INTENT`;
  ручное отключение синхронно сбрасывает durable-флаг и не может воскресить остановленный VPN.
...
- Обновлены Rust-зависимости `mio 1.2.2 → 1.2.3`, `rcgen 0.14.9 → 0.14.10`,
  `chacha20 0.10.1 → 0.10.2`, `log 0.4.33 → 0.4.34` и `argon2 0.5.3 → 0.6.0`, а CI
  использует `docker/setup-qemu-action 4.3.0`. Генерация новых Argon2id PHC-хешей
  сведена в общий helper с закреплённым профилем `m=19456, t=2, p=1` и новой случайной
  солью для каждого пароля; существующие хеши Argon2 0.5 продолжают проверяться без
  сброса учётных данных.
- UDP datapath на Linux и Android получает до 32 уже ожидающих датаграмм одним `recvmmsg` на
  ...
  её заполнения. Клиентские burst-последовательности `DATA_FRAG` отправляются через `sendmmsg` с
  корректной обработкой частичной отправки; Windows, macOS и iOS используют совместимый цикл
  одиночных вызовов, который не скрывает постоянные socket-ошибки после частичной пачки. Размер
  фиксированного recycler-пула по-прежнему ограничивает память и число принятых датаграмм, а сервер
  раз в 10 секунд журналирует ненулевые UDP drops отдельно для `queue_full`, `pool_exhausted`,
  `oversize`, `socket_error` и `recycler_closed`. Короткий same-window old/new A/B на двухъядерной
  лабе не подтвердил воспроизводимое изменение goodput или CPU: первый проход дал −1,5% upload,
  −3,7% download и видимое снижение CPU принимающего процесса на 10–12%, а повторный per-thread
  проход — +6,7% upload, +1,9% download без снижения CPU. `perf trace` независимо подтвердил саму
  работу `recvmmsg`: средняя успешная пачка под основным потоком составила 3,55 датаграммы на
  серверном upload ingress и 4,46 на клиентском download ingress. Обычный egress при этом всё ещё
  выполнял один `sendto` на запись (около 290–298 тысяч вызовов за тестовое окно); `sendmmsg`
  использовался только для готовой последовательности `DATA_FRAG`. Поэтому изменение фиксируется
  как проверенный receive-syscall batching и запас для дальнейшей оптимизации, без заявления о
  доказанном росте скорости или снижении CPU.
- Следующий этап снял найденное выше ограничение обычного UDP egress: при активном Recordizer
  клиент и сервер без дополнительного coalescing-таймера забирают до 32 уже ожидающих
  TUN/внутренних записей и отправляют готовые датаграммы одним `sendmmsg` на Linux/Android.
  Сохраняются порядок PacketCodec, QUIC packet number, DATA_FRAG и roaming CID; сервер привязывает
  пачку к одному egress snapshot, capped-пользователи продолжают точный per-record pacing,
  частичная отправка повторяет только непринятый хвост, а EMSGSIZE сохраняет PMTU/DATA_FRAG
  fallback. Windows, macOS и iOS используют тот же API с последовательным fallback. Чередующийся
  A/B, где baseline уже содержал receive batching, дал медиану upload 320,8 → 694,7 Мбит/с
  (+116,5%) при относительном CPU клиента −13,2% и download 359,8 → 697,5 Мбит/с (+93,9%) при
  относительном CPU серверного worker −7,9%. Kernel tracepoints подтвердили механизм: в upload
  `361 323 sendto` сменились на `37 328 sendmmsg` (`sendto=0`), а в download `365 752 sendto` —
  на `33 966 sendmmsg` и 104 служебных `sendto`. GSO/GRO не включались: выигрыш получен без
  требования одинакового размера датаграмм и не ослабляет рандомизацию размеров Recordizer.
- Desktop- и mobile-клиенты теперь журналируют фактический ABI загруженного Rust-ядра в
  формате major.minor и явно отделяют его от compatibility floor; документация Windows,
  macOS и iOS синхронизирована с текущим ABI 1.15.
- Android и iOS больше не расходуют reconnect-backoff, пока отсутствует пригодная физическая
  Wi-Fi/LTE-сеть, и возобновляют подключение сразу после появления carrier. Android сохраняет
  fail-closed TUN и передаёт его новой native generation, если адреса, DNS, маршруты, MTU,
  per-app и leak policy не изменились; это сохраняет старые TCP-сеансы приложений без
  повторного системного маршрута и DNS-reset. iOS при эквивалентном NetworkPlan не применяет
  повторно идентичные NetworkExtension settings и переносит только ещё не принятые ядром
  uplink-пакеты в ограниченном буфере: TCP — до 120 секунд, UDP — до 2 секунд, прочие — до
  5 секунд. Смена внутреннего адреса, маршрутов или DNS очищает очередь и принудительно
  применяет новый план, поэтому данные старой сессии не попадут в другую сеть.
- Исправлен desktop `route_file`: ключ можно повторять для нескольких файлов; все источники
  читаются по порядку, маршруты канонизируются и дедуплицируются. Поддерживаются обычные
  IPv4/IPv6 CIDR, OpenVPN `route <CIDR>`, host route `route <IPv4>` (явный `/32`),
  `route <IPv4> <netmask> [gateway] [metric]` и `route-ipv6 <CIDR>`. Экспортированные
  gateway/metric принимаются для совместимости, но next-hop остаётся аутентифицированным
  tunnel gateway.
  Неверная строка, некорректная или разрывная IPv4-маска, недоступный файл или превышение
  общего лимита 250 000 маршрутов останавливает подключение fail-closed. Чтение и установка
  больших списков Windows/macOS прерывается кнопкой отключения и пишет один итоговый log
  вместо тысяч синхронных UI-сообщений. Windows больше не создаёт дополнительный broadcast
  `/32` для каждой импортированной подсети; системные host routes самого tunnel pool сохранены.
  Android/iOS сохраняют все повторяющиеся `route_file` при переносе INI-профиля.
  Двуязычная инструкция дополнена пошаговым примером, правилами абсолютных путей с пробелами,
  таблицей поддерживаемого синтаксиса и описанием fail-closed поведения.
  Регрессионный гейт воспроизводит материалы из issue #69: наборы из 14 113 сетей в CIDR- и
  OpenVPN-форматах, отсутствие синтезированных для импортов `/32`, объединение двух файлов,
  отмену чтения списка и полный цикл очистки 14 113 Windows routes с повтором после временной ошибки.
- Явное `lport = 0` снова означает автоматический ephemeral-порт, как отсутствие ключа.
- Документация и сайт фиксируют актуальный статус Keenetic/OpenWrt: оба client-only варианта проверены на реальных роутерах и работают; требования к архитектуре, TUN и платформенным именам интерфейсов сохранены.
...
- Реализованы серверные `KICK` и `NOTICE` с отдельным двусторонним capability
  `MANAGEMENT_V1`: старые клиенты не получают неизвестные кадры, а отключение roaming в профиле
  не отключает административное управление сессией. Payload типизирован, ограничен 512 байтами
  UTF-8 и принимается только внутри аутентифицированной `PacketCodec`-записи.
  Native-адаптер дополнительно подтверждает `MANAGEMENT_EVENTS`, поэтому новый core безопасно
  работает со старым GUI; уже ожидающий `KICK` не может быть вытеснен последующим `NOTICE`.
  Минимальная сборка `--no-default-features --features client` сохраняет безопасный fallback
  без management/roaming и снова проходит отдельный compile gate.
- Административное отключение, превышение квоты и истечение срока сначала отправляют терминальный
  `KICK` с причиной, затем сервер всё равно авторитетно отзывает сессию. `NOTICE` предупреждает о
  80/95% квоты и сроке 7/1 день без спама в каждом usage sweep; не поставленное в очередь
  уведомление повторяется на следующем проходе.
- Android, iOS, Windows, macOS и Linux обрабатывают события через общее Rust-ядро ABI 1.15.
  `NOTICE` попадает в журнал/системное уведомление, а `KICK` останавливает generation; флаг
  `reconnect_allowed=false` запрещает reconnect.
- Терминальный `KICK` больше не конкурирует с data queue и последующим отзывом сессии: TCP и UDP
  получили отдельную приоритетную writer-очередь, один message ID с `ACK_REQUIRED` и ожидание
  аутентифицированного клиентского ACK. UDP повторяет KICK трижды, roaming принимает ACK только
  с committed path, а клиент подтверждает кадр до остановки generation. Стартовый UDP PMTU-loop
  теперь также распознаёт KICK вместо отбрасывания всех корректных не-PMTU датаграмм.
...
- Dual-stack DNS gate проверяет IPv4- и IPv6-listeners встроенного прокси, A/AAAA через туннель,
  отдельные IPv4/IPv6 upstream-прогоны, применение обоих resolver-адресов и catch-all domain `~.`
  через `resolvectl`, а также обязательный clean revert без изменения resolver хоста лабы.
- MTU gate разделён на два реальных режима: auto-PMTU на outer MTU 1280 проверяет независимые
  uplink/downlink budgets и перенос 1400-байтного inner IPv6 через DATA_FRAG; explicit MTU 1280
...
- RU/EN документация синхронизирована с исходным ABI 1.15, текущими native cores, результатами H2 и
...
- Опубликован полный сравнительный лабораторный прогон 34 VPN-режимов: 25 masked-режимов
  проверены в трёх проходах, зафиксированы IPv4/IPv6 TCP/UDP, CPU/RSS, kernel counters,
  повторяемость и границы DPI-интерпретации. Все 12 профилей Qeli работали с обязательным
  Recordizer; среднее TCP `P=4` составило 1220 Мбит/с, а для пяти быстрых TCP-профилей —
  1767 Мбит/с. Полный датированный отчёт опубликован зеркально на русском и английском в
  `docs/{ru,eng}/reports/benchmarks/` и связан с корневым README и индексами документации.
- Генератор сайта берёт JSON-LD `softwareVersion` и release download URL из
  `site/assets/site.json`, поэтому публичная structured data больше не остаётся на старой версии.
...
- Gradle на Android-лабе ограничен одним worker и heap 1536 МБ: полная offline release-сборка
  больше не уходит в swap-thrashing на 2 ГБ VM, а при ошибке скрипт выводит расширенный хвост лога.
...
- UDP-служебные кадры клиента (`CLIENT_INFO` с версией/платформой, MTU и payload-budget) теперь проходят через согласованный `PACKET_MUX_V1` и немедленно отправляются вместе с несколькими идемпотентными повторами. Сервер больше не отбрасывает версию новых Android/iOS/Windows/macOS/Linux-клиентов в UDP-профилях с recordizer; панель корректно показывает, в том числе, клиент 0.8.0.
- Панель исключает выключенные профили из runtime-сводки роуминга, поэтому они больше не отображаются жёлтыми как «нет данных процесса». Навигация профилей конфигурации использует карточки естественной ширины с переносом целыми элементами: названия отображаются полностью, а выбранный профиль выделяется мягким фоном и рамкой без внутренней синей полосы. Транспорт обозначается явными текстовыми badges `TCP`/`UDP`.
- Quick Start различает успешный restart и сохранённый профиль без подтверждённого data-plane,
  а wildcard bind `0.0.0.0`/`::` больше не показывается как адрес подключения. Обычные Add/Clone
  ищут свободную комбинацию port/interface/non-overlapping IPv4 `/24` в допустимом диапазоне и
  не создают невалидный `10.8.256.0/24`; новое имя профиля также проверяется на уникальность.
- Локальные журналы Android, iOS, Windows и macOS снова показывают безопасно очищенное имя
  пользователя при начале подключения и после успешной AUTH. Серверные и системные Linux-журналы
  сохраняют стабильный псевдоним `user#…`, поэтому исправление не возвращает персональные данные
  в потенциально экспортируемую диагностику. Общий `NetworkPlan.connection_log` теперь на всех
  клиентах явно сообщает серверную `recordizer.policy` и фактический результат согласования:
  `PACKET_MUX_V1 active` либо `legacy packet-per-record` с указанием TCP/UDP.
- Полностью локализованы новые элементы панели 0.8.0: Session Roaming, PACKET_MUX recordizer, dual-stack/IPv6, Reality/H2 и транспортная диагностика. Статический release gate теперь проверяет русский перевод всего видимого текста всех шаблонов, а не только кнопок и списков.
- Канонический backend-default нового профиля теперь включает `obf.recordizer.policy = prefer`;
  форма панели и Quick Start используют его напрямую, сохраняя `off` только как совместимый
  schema-baseline старых конфигов без ключа. Quick Start `reality-tls` записывает канонический
  `obf.mode = reality-tls` и тем самым закрепляет настоящий HTTP/2 carrier вместо legacy alias.
- Все выпадающие списки панели получили единое выравнивание по левому краю и отдельный правый отступ под стрелку. Компактные фильтры автоматически измеряют самый длинный пункт после локализации и динамического наполнения, поэтому русские подписи больше не обрезаются шириной, подобранной под английский текст; gate запрещает возвращать фиксированную ширину или центрирование select-полей.

- Закрыта гонка break-before-make между потерей старого carrier и подготовкой нового пути во всех
  клиентах. Общий UDP actor больше не завершает generation при первой ошибке отправки в исчезнувший
  интерфейс: он на ограниченное время приостанавливает старый uplink, запрашивает свежий PathUpdate
  и принимает candidate от Android, iOS, Windows, macOS либо Linux; после 15 секунд без commit
  выполняется обычный fail-closed reconnect. TCP больше не считает мгновенный отказ постановки
  `COMMIT_PATH` для superseded candidate неоднозначным post-commit результатом: фаза меняется только
  после фактической выдачи команды платформе. Android и iOS не добавляют 350 мс debounce после уже
  подтверждённой потери carrier, сохраняя его для обычного make-before-break callback burst.


- Устранена асимметрия break-before-make при переходе с Wi-Fi на мобильную сеть. Android больше
  не уничтожает активную generation после `onLost(Wi-Fi)` до появления LTE: TUN отвязывается от
  исчезнувшего carrier, замена ожидается до 5 секунд и проверяется каждые 250 мс, а roaming
  запускается только после появления пригодного IPv4/IPv6-адреса. Windows и macOS используют
  такой же ограниченный settle в общем desktop lifecycle; повторные GUI/daemon-события
  схлопываются и не продлевают deadline. iOS удерживает generation на промежуточном
  `NWPath.unsatisfied`, отменяет ожидание при готовом пути и только после 5 секунд без замены
  запрашивает полный reconnect. Linux sampler уже сохранял generation без маршрута и продолжает
  поиск до двух стабильных выборок нового пути; его симметричное поведение подтверждено аудитом.
...
**клиент для OpenWrt** (procd + UCI + LuCI), **router-режим
...
- **Нативный OpenWrt-пакет клиента** ([qeli-openwrt/](qeli-openwrt/)): тот же
  ...
  кросс-собранные бинари под **aarch64 / armv7 / mipsel / x86_64**. Текущая линия
  проверена на реальном OpenWrt-устройстве и работает. Установка — [qeli-openwrt/INSTALL.md](qeli-openwrt/INSTALL.md).

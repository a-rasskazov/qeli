...
слушателем. Правила NAT второго профиля сняты, запись из реестра убрана, первый профиль не
затронут.

> **Остаётся: устройство переживает откат.** `ip tuntap del` теперь отрабатывает успешно, но
> интерфейс всё равно живёт — каждая очередь отдаёт `libc::dup` своего fd блокирующему
> читателю, который навсегда паркуется в `read()`, а устройство без `IFF_PERSIST` исчезает
> только когда закрыт последний дескриптор. Закрывать их из guard нельзя: закрытие fd, на
> котором другой поток стоит в `read()`, — это use-after-free по номеру дескриптора. Нужен
> сигнал остановки на очередь, а он трогает горячий путь пакетов, и в этот раунд такое не
> вносится. Пока что снимаются правила и реестр, устройство — нет, и в логе это сказано прямо.
...
**Флаг TC на своём ответе намеренно НЕ ставится**, вопреки рекомендации аудита: прокси биндит
только UDP-сокет, TCP-слушателя у него нет, поэтому `TC=1` отправил бы клиента на порт, где
никто не ответит, — то есть превратил бы работающий сейчас резолв в отказ. Из двух доступных
без TCP-слушателя поведений пересылка целого ответа — то, которое резолвит имена. Причина
записана в коде рядом с отправкой.

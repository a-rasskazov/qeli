...
**Осталось:** кадр шлёт только Rust-клиент. C#/Kotlin/Swift пока показываются прочерком —
их порт вторым заходом, вместе с общей conformance-фикстурой на этот кадр (сейчас вектор
пиннится инлайн, как и у соседнего MTU-отчёта).
...
- **Отчёт теперь переотправляется на UDP.** Кадр не подтверждается по устройству протокола
  ...
  хранит наименьшее увиденное значение), поэтому он просто повторяется: в Rust на первых
  тиках простоя (~0/5/10 с), в Kotlin и C# — через 2 и 6 с. На TCP повторов нет: там
  ретрансмит даёт сам транспорт.

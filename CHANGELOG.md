...
В этом разделе будут накапливаться изменения следующей версии qeli 0.8.2.

- Исправлена навигация панели при `web.base_path`: ссылка «Панель» использует канонический корень без завершающего `/`,
  а прямой переход на `/prefix/` перенаправляется на `/prefix` вместо ответа 404.
- Исправлена условная валидация формы конфигурации панели: скрытые параметры выключенного
  DNS-прокси, веб-панели, Recordizer, `obfs` fronting и неактивного IPv6 address pool больше
  не мешают сохранить независимые изменения. При повторном включении функции её сохранённые
  параметры снова строго проверяются. Переключатели transport/IP mode/wire mode теперь сразу
  приводят зависимые параметры в согласованное состояние. Диапазоны Reality peek timeout,
  multipath streams, shaping и web session TTL согласованы между формой, серверным preflight
  и фактическим runtime.
- Карточка «Нагрузка хоста» на dashboard переработана в единый адаптивный операционный блок:
  состояние метрик, CPU/RAM, load average, память и диск, процесс qeli, WAN, TCP/UDP и аптайм
  сгруппированы по смыслу; штатное состояние не выводит отдельные индикатор и подпись, предупреждения
  используют те же пороги 90%, что и системный status API, а цвет RAM не смешивается с цветом предупреждения. Компоновка перестраивается по ширине самой карточки.
- Счётчики уровней на странице журнала панели стали интерактивными фильтрами с
  мультивыбором. Поиск, автообновление и исходные номера строк согласованы с фильтрами,
  а уровень определяется по полю severity, а не по словам «ERROR»/«WARN» внутри сообщения.
- Расширен контракт клиентских lifecycle-хуков `post_up`/`post_down` для standalone Linux,
  OpenWrt и Keenetic. Shell получает `$1=ifname`, `$2=gateway`, полный versioned `QELI_*`
  контекст фактически применённого dual-stack `NetworkPlan`, реальный carrier/uplink,
  согласованные Recordizer/roaming/data-plane параметры и terminal reason/error. Для сложной
  логики на время вызова создаётся защищённый JSON-файл `QELI_CONTEXT_FILE` (`0600`); секреты
  подключения в hook-контекст не попадают. Снимок обновляется при reconnect, `post_up` остаётся
  одноразовым, а `post_down` получает последнее успешно применённое поколение.
...
Полное двуязычное описание основных изменений и порядок обновления: [RELEASE_NOTES_0.8.1.md](release/RELEASE_NOTES_0.8.1.md).

0.8.1 закрепляет архитектурную переработку 0.8.0 для повседневной эксплуатации. Релиз закрывает
не одну отдельную ошибку, а четыре класса практических проблем: потолок UDP из-за syscall на каждую
датаграмму, разрывы сессий при реальной смене Wi-Fi/LTE, сложные IPv6/desktop-развёртывания и
неполные изменения административного состояния. Для qeli в целом это означает почти двукратный
рост измеренной скорости обычного UDP-пути, меньше пауз и переподключений, безопаснее маршрутизацию
и обслуживание и более одинаковое поведение сервера и всех клиентов.

- Подготовлен полный набор для Linux, DEB, Android, Windows, macOS, OpenWrt и Keenetic: 17 payload-файлов и `SHA256SUMS`.
...
- iOS атомарно откатывает модель профилей при ошибке сохранения, не предлагает JSON как формат импорта профиля и проверяет App Group/Keychain Sharing публичными capability-пробами в приложении и Packet Tunnel extension; сборка совместима с публичным iOS SDK без приватного `SecTask`. Offline IPA verifier сверяет срок provisioning profile, Team ID, App ID и entitlements каждого bundle; недостижимая legacy JSON/SSH-реализация `gen_client_links.py` удалена.
- В Android и iOS в «Свойствах соединения» отображаются фактически согласованные после handshake
  ...
  вместе с применённой политикой. Там же показываются согласованное семейство туннеля и реальный
  внешний carrier endpoint после DNS-разрешения; старые native core не подменяются догадками UI.
- Карточки ключей идентичности сервера в панели теперь полностью показывают длинные bind-адреса и адаптивно
  перестраивают адрес, публичный ключ и действия на узких экранах. IPv6 endpoint отображается однозначно как
  `transport://[address]:port`; bind-адрес можно скопировать отдельно.
- Во всех полях ввода панели placeholder теперь использует отдельный приглушённый цветовой токен
  для светлой и тёмной темы. Пример значения визуально заметно отличается от уже введённых данных;
  одинаковое отображение применяется ко всем `input` и `textarea`, а browser opacity нормализована.
...
- Хранилища профилей GUI и launchd-helper в macOS переведены на версионированный
  AES-256-GCM envelope с явной сигнатурой и аутентифицированным заголовком. Корректный legacy
  ciphertext больше нельзя ошибочно принять за JSON из-за случайного первого байта nonce;
  прежний GCM-формат и настоящий legacy JSON мигрируют автоматически, неизвестная версия
  отклоняется fail-closed, а повреждённый пользовательский файл восстанавливается из
  аутентифицированной `.bak`. Настройки Windows и macOS сохраняются атомарно с `WriteThrough`,
  резервной копией и quarantine повреждённого поколения.
- DHCP-сервер на Linux принимает broadcast DISCOVER через `0.0.0.0:67`, ограниченный конкретным
  TUN/TAP-интерфейсом посредством `SO_BINDTODEVICE`; небезопасный wildcard больше не задаётся в
  конфиге оператора. REQUEST переводит короткий OFFER в полный lease и каждое renewal продлевает
  срок, независимый reaper освобождает истёкшие адреса из общего пула. При выключенном встроенном
  DNS DHCP использует настроенные `dns.push_servers`, не навязывая публичные резолверы.
- Обновление DDNS/roaming allowlist Windows kill-switch сделано транзакционным: новый WinDivert
  gate открывается до закрытия старого, а firewall-правила и метаданные откатываются при ошибке.
  OpenWrt разрешает root-init и firewall defaults работать только с выделенными интерфейсами
  `qeli*` длиной до 15 безопасных символов, поэтому ACL на UCI `qeli` нельзя использовать для
  перепривязки NAT/forwarding к `br-lan`, `wan` или чужому TUN.
- Настройки Telegram/webhook теперь загружаются и сохраняются fail-closed: ошибка чтения не может
  быть перезаписана пустой формой, кэш согласуется с атомарной записью, обязательные поля
  проверяются, выключение Telegram удаляет сохранённый token, а переключатели connect/disconnect
  сохраняются. Webhook корректно формирует `Host` с нестандартным портом и поддерживает
  bracketed IPv6 literal URL.
- Panel backup учитывает фактические пути активного config, users, identity и TLS внутри
  `/etc/qeli`; для внешних/относительных путей он явно отказывается заявлять неполный архив
  успешным. Dashboard считает сокеты только data-plane worker и показывает CPU в шкале `top`.
  Anti-enumeration для неизвестных пользователей выбирает реальный Argon2id cost-профиль базы;
  исправлены сообщения CLI, preflight лабораторного config-теста и устаревшие описания Argon2,
  Reality TLS и wire modes в русской и английской документации.
...
- Обновление Windows per-app карты TCP/UDP endpoint → PID → executable теперь целиком строит новый
  снимок вне пути обработки пакетов и публикует его атомарно. Медленное чтение системных таблиц и
  путей процессов больше не удерживает WinDivert-классификацию и не создаёт секундные паузы трафика
  на системах с большим количеством сокетов, защитных фильтров или виртуальных интерфейсов.
  Подробности и сценарии воспроизведения: [issue #158](https://github.com/litvinovtd/qeli/issues/158).
  Спасибо [@ambyte](https://github.com/ambyte) за исходный репорт и
  [@cop30](https://github.com/cop30) за дополнительную диагностику виртуальных адаптеров и
  блокировки обновления ProcessAppMap.

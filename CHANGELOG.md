...
- **Новый внутритуннельный control-кадр `CTRL_CLIENT_INFO` (тип 2)**
  ...
  существовал — тот самый, которым с 0.7.13 передаётся отчёт о path-MTU, — поэтому добавлен
...
одновременно: у новых сессий в CLI и в `/api/clients` стоит `0.7.13/linux`, у 0.7.11 —

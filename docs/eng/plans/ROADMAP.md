...
Tracker — [AUDIT-2026-06-12.md](../archive/audits/AUDIT-2026-06-12.md).
...
details of the C# consolidation and Rust fixes — [REFACTOR-PLAN.md](../archive/plans/REFACTOR-PLAN.md).
...
- ✅ **Multi-queue TUN + the client `dev=`** (2026-06-06). Server: `tun.queues`
  ...
  [BENCHMARK.md](../reports/BENCHMARK.md)). Client: `dev=` in `[qeli]` (default `vpn0`) —
...
- ✅ **A full benchmark run of all 10 modes** (incl. `plain` + `reality-tls`) with
  process CPU/RSS metrics — see [BENCHMARK.md](../reports/BENCHMARK.md).
...
1. **Real REALITY** (a TLS 1.3 tunnel + proxying foreign parties to a real site) — the
   ...
   - ✅ **M2 (done 2026-06-05)** — a real browser TLS client, a pure-Rust `realtls`
     core (decided 2026-06-05, `docs/*/archive/plans/DESIGN-remaining.md`); interop with rustls proven:
   ...
   - ✅ **APPLICATIONS — the FFI realtls core** (the sans-IO core → Android + Windows +
     macOS; `docs/*/archive/plans/DESIGN-remaining.md`):
...
- ✅ **FFI panic-safety: client cdylibs use `panic = "unwind"`** (completed in 0.7.15,
  [TRANSPORT-CORE TC-0.1](../reference/TRANSPORT-CORE.md)). Android/Windows/macOS/iOS native build recipes

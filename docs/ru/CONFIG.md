...
Firewall-фейл-клоуз на клиенте (Linux/iptables, **только при full-tunnel**): пока туннель
лежит, весь egress кроме loopback / tun / DHCP / IP сервера блокируется, чтобы обрыв не
«протёк» на физический интерфейс. Включается ключом `kill_switch = true` в `[qeli]`.
...
**Сервер** — зарегистрировать выход за exit-пользователем и разрешить трафик клиент↔клиент:
```ini
[user:exit]
password_hash = ...
...
**Потребитель** (Win/любой клиент) — просто получает дефолт, про exit ничего не знает:
```ini
[qeli]
gateway = true
```
Кому давать выход — решается на сервере: пушьте дефолт (`route = 0.0.0.0/0` в `[user:*]`)
только нужным пользователям, тогда один exit-узел обслуживает выбранных, а не всех.
...
- WAN определяется автоматически (`ip route get 1.1.1.1`).

...
> зависимости, **спрашивает профиль** — `reality-tls` (по умолчанию; настоящий TLS 1.3 + H2 на
> TCP:443, мостит неавторизованные пробы на target), `fake-tls` (дешевле по CPU, хватает против пассивного
...
# транспорт-независимая маскировка границ пакетов/записей
...
Новые профили используют `obf.mode = reality-tls` на сервере и `mode = reality-tls` на клиенте. При server-first миграции новый сервер временно принимает legacy-сочетание `fake-tls + real_tls=true`.

| Режим | Когда |
|---|---|
| `fake-tls` | **по умолчанию.** Мимикрия под TLS 1.3, против пассивного/сигнатурного DPI. Хороший баланс. |
| `reality-tls` | максимальная текущая TCP-маскировка: REALITY TLS 1.3 + настоящий H2 carrier и bridging неавторизованных проб на target. Нужны `key` + `reality_sid` + совпадающий `sni`; сначала обновляется сервер. |
...
| QUIC-shape | для **UDP**-профилей (`obf.quic.enabled = true`); поверхностная совместимость, не настоящий QUIC/HTTP3. |

Подробное сравнение, REALITY-настройка (short_ids, handrolled), multipath-бондинг —
в [CONFIG.md](CONFIG.md). Бенчмарки всех режимов — [BENCHMARK.md](../reports/BENCHMARK.md).
...
(см. `docs/*/manuals/KEENETIC-DEPLOY.md`).

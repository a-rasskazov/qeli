...
Что уже работает: Rust-конфиг разбирает `[2001:db8::1]:443`, C#-клиент формирует корректную
`qeli://`-ссылку со скобками ([VpnConfig.cs](../../qeli-shared/QeliShared/Model/VpnConfig.cs)).
...
- ✅ **Обратный PMTU-канал (клиент сообщает серверу найденный path-MTU).**
  ...
  **Одно утверждение из обоснования отсрочки было просто неверным**, и это стоит оставить на
  ...
  Осталось: фрагментация отказывается работать с IPv4-заголовками, несущими ОПЦИИ (копирование
  при фрагментации задаётся каждой опцией отдельно; такие пакеты по-прежнему отбрасываются), и
  для IPv6 нужен аналог с ICMPv6 Packet Too Big, когда появится форвардинг IPv6.

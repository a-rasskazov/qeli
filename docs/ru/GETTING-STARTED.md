...
> **Как `share-link` узнаёт пароль.** В users-файле рядом с необратимым Argon2-хешем при
> заведении сохраняется обратимо зашифрованная копия пароля — из неё ссылка и собирается,
> поэтому конфиг можно переслать клиенту повторно в любой момент. Это тот же механизм, что
> у кнопки share/QR в панели, и содержимое ссылки строится тем же кодом.
>
> Если копии нет (пользователь заведён до появления этой возможности либо сменился ключ),
> команда **откажется** и предложит `--reset`: тогда генерируется новый пароль, сохраняется
> и печатается один раз, а **текущий конфиг этого пользователя перестаёт работать**. После
> `--reset` работающему серверу нужен `systemctl reload qeli`, иначе он продолжит проверять
> старый пароль (панель делает это сама, у CLI такого канала к воркеру нет).

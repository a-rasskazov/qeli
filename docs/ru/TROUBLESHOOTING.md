...
**Исправлено** в 0.7.13: в течение 30с после пробуждения или смены сети счётчик попыток
ограничен (повтор не реже чем раз в ~4с), поэтому реконнект происходит сразу, как только
сеть готова. Бэкофф остаётся в силе там, где он и нужен, — когда сервер действительно
лежит. Отдельных действий не требуется, нужен только клиент 0.7.13+.

Если задержка сохраняется на 0.7.13+ — нужен клиентский лог от пробуждения до
`Connected` (вкладка **Журнал** → **Copy log**): в нём видно, на чём именно уходит время.
